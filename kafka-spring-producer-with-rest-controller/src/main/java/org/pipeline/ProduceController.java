...
//            @RequestHeader("user-agent") String userAgentName,
        ...
        Date now = new Date();
        Gson gson = new Gson();
//        UserEventV0 userEventV0 = new UserEventV0(sdfDate.format(now), userAgentName, colorName, userName);
        UserEventV0 userEventV0 = new UserEventV0(sdfDate.format(now),colorName, userName);
        String jsonColorLog = gson.toJson(userEventV0);
        kafkaTemplate.send("select-color4", jsonColorLog).addCallback(
